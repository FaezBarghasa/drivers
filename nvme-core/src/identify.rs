...
impl IdentifyControllerData {
    pub fn model_number(&self) -> String {
        String::from_utf8_lossy(&self.model_no).trim().to_string()
    }
    pub fn serial_number(&self) -> String {
        String::from_utf8_lossy(&self.serial_no).trim().to_string()
    }
    pub fn firmware_revision(&self) -> String {
        String::from_utf8_lossy(&self.firmware_rev).trim().to_string()
    }
    pub fn nvm_ns_count(&self) -> u32 {
        self.nn
    }
    
    // Additional accessors needed by nvme-driver
    pub fn max_sq_entries(&self) -> u16 {
        self.maxcmd
    }
}
#[derive(Clone, Copy)]
...
impl IdentifyNamespaceData {
    pub fn size(&self) -> u64 {
        self.nsze * self.lba_format_support[self.formatted_lba_size_idx()].lba_data_size().unwrap_or(512)
    }
    pub fn block_size(&self) -> u64 {
        self.lba_format_support[self.formatted_lba_size_idx()].lba_data_size().unwrap_or(512)
    }
    pub fn blocks(&self) -> u64 {
        self.nsze
    }
    pub fn optimal_write_size(&self) -> Option<u32> {
        Some(self.nows as u32 * (self.block_size() as u32))
    }
    pub fn max_transfer_size(&self) -> Option<u32> {
        // MDTS is in controller, not namespace. Returning None implies default.
        None 
    }
